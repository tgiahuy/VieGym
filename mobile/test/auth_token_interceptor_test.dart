...
void main() {
  group('TokenStorage (ADR-004) Unit Tests', () {
    ...
    test('Access token is stored in memory and refresh token in secure storage', () async {
    ...
    test('rotateTokens updates both access token and secure refresh token', () async {
      await storage.saveTokens(
    ...
    test('clearTokens completely wipes in-memory and secure credentials', () async {
      await storage.saveTokens(
  ...
  group('AuthInterceptor Single-Flight & Concurrency Tests', () {
    test('Attaches Authorization Bearer header when access token is present', () async {
    ...
    test('Single-Flight: 5 concurrent 401 requests trigger ONLY 1 refresh API call', () async {
      final storage = DefaultTokenStorage();
      await storage.saveTokens(
    ...
    test('Refresh failure triggers onRefreshFailed and clears credentials', () async {
      final storage = DefaultTokenStorage();
      await storage.saveTokens(
        accessToken: 'expired_access_token',
      ...
      final interceptor = AuthInterceptor.withStorage(
        ...
        refreshApi: (_) async => throw Exception('Refresh token expired/revoked'),
    ...
    test('401 on /auth/refresh endpoint does not trigger recursive refresh loop', () async {
      int refreshCallCount = 0;
      final storage = DefaultTokenStorage();
  ...
  group('AuthController & TokenStorage Integration Tests', () {
    ...
    test('restoreSession restores authenticated user when refresh token exists in secure storage', () async {
      final storage = DefaultTokenStorage();
      await storage.saveTokens(
