...
void main() {
  ...
  group('WorkoutSessionController exercise management tests', () {
    ...
    testWidgets(
      'WorkoutBuilderScreen renders days, allows adding day and editing exercise targets',
      (tester) async {
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets(
      'E2E Flow 2: Exercise -> Favorite -> Program -> Schedule -> Session -> Log/PR',
      (tester) async {
        tester.view.physicalSize = const Size(1080, 2400);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        final testEx = exerciseCatalog.firstWhere(
          (e) => !favNotifier.isFavorite(e.id),
        );
        ...
        expect(
          container.read(workoutSessionProvider).exercises.isNotEmpty,
          isTrue,
        );
        ...
        expect(
          container.read(workoutScheduleProvider).history.first.workoutName,
          'E2E Full Body',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          container.read(exerciseCatalogControllerProvider).query,
          'Bench',
        );

        controller.setMuscleGroup(1);
        expect(
          container.read(exerciseCatalogControllerProvider).muscleGroupId,
          1,
        );

        controller.setEquipment(2);
        expect(
          container.read(exerciseCatalogControllerProvider).equipmentId,
          2,
        );
    ...
    test(
      'ExerciseApiSummary and ExerciseApiDetail map JSON and convert to ExerciseDefinition correctly',
      () {
        final summaryJson = {
          'id': 101,
